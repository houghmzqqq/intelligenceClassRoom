...
<div class="sidebar-nav">
    <ul>
        ...
        <li><ul class="accounts03-menu nav nav-list collapse in">
            <li class="active"><a href="${pageContext.request.contextPath}/attendance/toAttenView"><span class="fa fa-caret-right"></span>考勤机列表</a></li>
...
<div class="content">
    ...
    <div class="main-content">
        ...
        <ul class="nav nav-tabs">
          <li class="active"><a href="#home" data-toggle="tab">考勤机信息</a></li>
        </ul>
            ...
            <div class="col-md-4">
                ...
                <div id="myTabContent" class="tab-content">
                    <div class="tab-pane active in" id="home">
                        <form method="post" id="tab" action="${pageContext.request.contextPath}/attendance/update">
                            <input type="hidden" name="attendanceMachineId" value="${amPO.attendanceMachineId}">
                            <div class="form-group">
                                <label>考勤机IP</label>
                                <input name="IP" value="${amPO.IP}" type="text" readonly unselectable="on" class="form-control">
                            </div>
                            <div class="form-group">
                                <label>考勤机端口</label>
                                <input name="port" value="${amPO.port}" type="text" readonly unselectable="on" class="form-control">
                            </div>
                            <div class="form-group">
                                <label>教室编号</label>
                                <select name="roomId" class="form-control">
                                    ...
                                    <c:forEach var="classroomPO" items="${classroomPOS}">
                                        <option <c:if test="${classroomPO.roomId==amPO.classroomPO.roomId}">selected="selected"</c:if>
                                                value="${classroomPO.roomId}">${classroomPO.roomId}</option>
                                    </c:forEach>
                                </select>
                            </div>
                            <div class="form-group">
                                ...
                                <select name="status" class="form-control">
                                    <option <c:if test="${amPO.status=='unusable'}">selected="selected"</c:if>
                                            value="unusable">unusable</option>
                                    <option <c:if test="${amPO.status=='usable'}">selected="selected"</c:if>
                                            value="usable">usable</option>
                                </select>
                            </div>
                            <%--<div class="form-group">
                                <label>选择开学日期</label>
                                <input name="RFID" value="2017-09-04" type="text" required="required" class="form-control">
                            </div>--%>
                            <div class="btn-toolbar list-toolbar">
                                <button class="btn btn-primary"><i class="fa fa-save"></i> 确认注册</button>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <p class="error-text"><i class="fa fa-warning modal-icon"></i>确认要删除该考勤机吗？<br>该操作不能够撤销。</p>
                    </div>
                    <form action="${pageContext.request.contextPath}/attendance/toDelete?attenId=${amPO.attendanceMachineId}">
                        <div class="modal-footer">
                            <button class="btn btn-default" data-dismiss="modal" aria-hidden="true">取消</button>
                            <button type="submit" class="btn btn-danger" data-dismiss="modal">删除</button>
                        </div>
                    </form>
                </div>
            </div>
